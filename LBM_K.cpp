...
// Calcula a distribuição de equilíbrio simplificada (sem os termos de segunda ordem)

void dist_eq_simplified ( double, double, double, double, double*, double*, double*, double );
...
void bond_eqdvnull_rho_x ( int*, double*, double*, int, double, int, int, int, double*, double );

//	Calcula a permeabilidade intrínseca

void intrinsic_permeability ( double, double, double, double, double, int, int, int, int  );
...
int main ()
{
    ...
    double ro_ini;      // Densidade (partículas/sítio)
    ...
    read_data ( nome_geo, ftesc, npassos, numarq, tau, visc, ro_ini );
    ...
    if ( c_int == 110 )
    {
        for ( int pto = 0; pto < ptos_meio; pto++ )
        {
            ...
            dist_eq ( vx_ini, vy_ini, vz_ini, ro_ini, ini_c, N, W, one_over_c_s2 );
        }
    ...
    for (unsigned int passo = inicio; passo < fim; passo++ )
    {
		...
		#pragma omp sections
        {
			...
			#pragma omp section
			{
				...
				bond_eqdvnull_rho_x ( ini_meio, ini_N, ini_c, pos_x, ro_ini, nx, ny, nz, W,
									  one_over_c_s2 );
			}
		...
		#pragma omp parallel for reduction(+:soma_mx) reduction(+:sum_force)
        ...
        {		
            ...
            if ( passo < 5 * nx ) 
            {
				...
				dist_eq_simplified ( vx_acc, vy_acc, vz_acc, rho, ini_c, f_eq, W, one_over_c_s2 );

				for (int i=0; i<nvel; i++)
				{
					N[i] = N[i] + ( f_eq[i] - N[i] ) * inv_tau;
				}
			}
			else
			{
        ...
        if ( passo % ( 100 ) == 0 && passo > 0 )
        {
            intrinsic_permeability ( soma_mx, sum_force, ftesc, phi, visc, ptos_meio, nx, passo,
                                     D_caract);
...
void dist_eq_simplified ( double vx, double vy, double vz, double rho, double *ini_c, 
							double feq[nvel], double W[nvel], double one_over_c_s2 )
{
    double *c;
...
void bond_eqdvnull_rho_x ( int *ini_meio , double *ini_f, double *ini_c, int posx, double rho,
                           int nx, int ny, int nz, double W[nvel], double one_over_c_s2 )
{
    int infx;
    int x = posx;
...
void intrinsic_permeability ( double soma_mx, double Q_lost, double ftesc, double phi, double visc,
